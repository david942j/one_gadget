  ...
  module Emulators
    ...
    class X86 < Processor
      ...
      def inst_xor(dst, src)
        ...
        raise Error::InstructionArgumentError, 'xor operator only supports dst = src' unless dst == src
      ...
      def inst_sub(tar, src)
        src = OneGadget::Emulators::Lambda.parse(src, predefined: registers)
        raise Error::InstructionArgumentError, "Can't handle -= of type #{src.class}" unless src.is_a?(Integer)
