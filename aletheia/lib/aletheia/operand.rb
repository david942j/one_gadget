...
module Aletheia
  ...
  Operand = Struct.new(:reg, :imm, :deref, :cast_bits) do
  ...
  module Operand_
    ...
    def parse(str)
      ...
      return Operand.new(nil, 0, deref, cast) if s == 'NULL'
      return Operand.new(nil, Integer(s), deref, cast) if s.match?(/\A-?(0x[0-9a-fA-F]+|\d+)\z/)
      ...
      Operand.new(m[1], imm, deref, cast)
    end
  end
end
